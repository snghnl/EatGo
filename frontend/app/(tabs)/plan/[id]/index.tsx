...
export default function PlanDetailScreen() {
    ...
    useEffect(() => {
        ...
        const newCourseData: TravelCourse = {
            id: String(id),
            start_date: startDate || "25.07.05", // SOURCE: URL 파라미터(임시값)
            end_date: endDate || "25.07.07", // SOURCE: URL 파라미터(임시값)
            destination: destinations || "전주", // SOURCE: URL 파라미터(임시값)
            title,
            routes: [],
        };
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <Header
                ...
                subtitle={courseData.destination || ""}
            />
...
export default function PlanDetailScreen() {
    const { id, startDate, endDate, destinations, foods, isNew } = useLocalSearchParams<{
        id: string;
        startDate?: string;
        endDate?: string;
        destinations?: string;
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            {isNewCourse ? (
                <View style={styles.addRecommendationContainer}>
                    <AddCourseCard onPress={handleNewCourseRecommendationPress} text="추천 여행 경로 확인하기" />
                </View>
            ) : (
                <ScrollView style={styles.scrollContainer}>
                    {/* 기존 코스의 경우 추천 경로 저장하기 버튼 */}
