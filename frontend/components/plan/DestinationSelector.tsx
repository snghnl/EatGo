...
export const DestinationSelector: React.FC<DestinationSelectorProps> = ({
    ...
    const currentItems = useMemo(() => {
        if (step === "province") {
            return (
                provinces?.filter((p) => p.is_active)?.map((p) => p.name) || []
            );
        } else {
            return (
                districts?.filter((d) => d.is_active)?.map((d) => d.name) || []
            );
        }
    ...
    const loading =
        provincesLoading || (step === "district" && districtsLoading);
